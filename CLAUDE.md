...
## Running everything on the host

This is the one thing to understand before touching settings, models, or the compose file.

1. **`.env`** (copy `.env.example`) — read by `python-dotenv` at the top of `config/settings.py`, no-op if absent. Two independent flags live here:
   - `DJANGO_DB_HOST` (+ `DJANGO_DB_NAME`/`_USER`/`_PASSWORD`/`_PORT`) — point at any real Postgres (this machine uses a pre-existing `pg_dev` container from another project). Without it, `DATABASES` falls back to sqlite. `USE_POSTGRES = bool(os.environ.get("DJANGO_DB_HOST"))`.
   - `DJANGO_USE_GIS=1` — additionally loads `django.contrib.gis` + `fleet` into `INSTALLED_APPS` and switches the DB backend to `django.contrib.gis.db.backends.postgis`. `USE_POSTGIS = USE_POSTGRES and bool(os.environ.get("DJANGO_USE_GIS"))`.
2. **GDAL/GEOS/PROJ** — the one thing `pip` can't provide; `django.contrib.gis` loads them via `ctypes`, so they must exist as *system* libraries wherever `DJANGO_USE_GIS=1` runs. Installed once with `sudo apt-get install -y gdal-bin libgdal-dev libgeos-dev libproj-dev` (this used to be the Dockerfile's entire reason for existing — a host with `sudo` doesn't need a container just for this). No matching pip package is needed or installed.
3. **`requirements.txt`** — one file, includes everything: Django/DRF/Celery/JWT, `psycopg[binary]` (Postgres), `pymongo` (Q9), `google-cloud-pubsub` (Q10). None of those three need GDAL themselves, which is why they were always safe to put here.
4. **Redis** — `redis://localhost:6379/15` (DB 15, to avoid colliding with unrelated keys already in DB 0 on this machine), backs Celery.

With `.env` + GDAL in place, every `demo_*` command across `payments/` and `fleet/` (Q1-Q14) runs the same way: `python manage.py demo_whatever`. Nothing here needs `sudo docker`.

```bash
docker compose up -d          # only mongo + pubsub-emulator, for Q9/Q10
docker compose down           # add -v to also drop seeded data
python manage.py runserver    # for the Postman collection, or the webhook curl example in Q4
```
...
- **`config/`** — the one Django project. `settings.py` loads a host-only `.env` via `python-dotenv`, then branches `INSTALLED_APPS`/`DATABASES` on `USE_POSTGRES`/`USE_POSTGIS` (see "Running everything on the host" above). `celery.py` is the Celery app entrypoint (`config.celery.app`), imported in `config/__init__.py` so `@shared_task` works app-wide.
...
- **`payments/management/commands/` and `fleet/management/commands/`** — one `demo_*` command per interview question. Each is a complete, idempotent, rerunnable proof: it seeds/clears its own data, runs the real mechanism, and prints real captured output (query counts via `CaptureQueriesContext`, raw `EXPLAIN` output, actual HTTP status codes, actual redelivered Pub/Sub `message_id`s). When adding a new demo, follow this pattern — seed inside the command, assert nothing that wasn't actually observed. **Cleanup must be scoped to the command's own rows** (`Wallet.objects.filter(owner_id__startswith="...")`, never `Wallet.objects.all().delete()`) for any model other demos or real accounts also write to — the DB is a real, shared, persistent Postgres, not a throwaway sqlite file recreated per run. (`fleet/`'s `VehiclePosition`/`DepotBay` are the one exception: nothing else in the repo touches those two models, so an unscoped delete there is fine.) `demo_django_signals`, `demo_drf_internals`, and `demo_auth_api` additionally accept `--keep-data` to skip their final cleanup, for when you actually want to inspect the seeded rows instead of just reading stdout.
- **`postman/`** — a Postman collection (`example-django-auth-api.postman_collection.json`) exercising the register/login/me/refresh/logout + wallet CRUD flow as real HTTP requests against `python manage.py runserver`, no Docker `app` container involved. Verified with `npx newman run ...` — see `postman/README.md` for the run order and why it matters (folder 3's 403 checks need folder 2 to not have already deleted the wallet, hence the dedicated cleanup folder at the end). Folder `0. My Account (ali)` logs into a real, persistent account (`ali`, `my_username`/`my_password` variables) instead of a throwaway one — run it, then any other request in the collection can be run individually against that account.
...
- **`requirements.txt`** is the only requirements file — Django/DRF/Celery/JWT plus `psycopg[binary]` (Postgres), `pymongo` (Q9), and `google-cloud-pubsub` (Q10), none of which need GDAL themselves. The one thing pip genuinely can't provide is `django.contrib.gis` (Q7/Q8's `fleet/`) needing the GDAL/GEOS/PROJ *system* libraries — see "Running everything on the host" above for the one-time `apt-get install` that covers it.
